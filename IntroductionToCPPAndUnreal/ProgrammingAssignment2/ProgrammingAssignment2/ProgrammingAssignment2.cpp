...
int main()
{
    ...
    while (Input[0] != 'q')
    {
        ...
        // comments as needed
        float diffX = Point2X - Point1X;
        float diffY = Point2Y - Point1Y;

        float deltaX = abs(Point1X - Point2X);
        float deltaY = abs(Point1Y - Point2Y);

        float distance = sqrt((deltaX * deltaX)+(deltaY * deltaY));

        float result = atan2(diffY, diffX) * 180 / M_PI;
